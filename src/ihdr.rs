...
pub fn get_color_mode(bit_depth: u8, color_type: u8) -> io::Result<PartialColorMode> {
    use ColorMode::*;
    ...
    match (color_type, bit_depth) {
        ...
        (6, 8) => Ok(Full(RGBA8)),
        (6, 16) => Ok(Full(RGBA16)),
        _ => format_error("Invalid bit depth and color mode combination"),
    }
}
...
pub fn get_interlace_method(interlace_method: u8) -> io::Result<InterlaceMethod> {
    match interlace_method {
        0 => Ok(InterlaceMethod::NoInterlace),
        1 => Ok(InterlaceMethod::Adam7),
        _ => format_error("Invalid interlace method"),
    }
}

pub fn load_ihdr(file: &mut File) -> io::Result<(u32, u32, PartialColorMode, InterlaceMethod)> {
    let (mut chunk, _, chunk_type) = ChunkReader::new(file)?;
    if *chunk_type != *b"IHDR" {
        return format_error("First chunk is not IHDR");
    }
    let width = chunk.read_u32()?;
    println!("Width: {}", width);
    if width == 0 {
        return format_error("Width is zero");
    }
    if width > 0x7FFFFFFF {
    ...
    if height == 0 {
        return format_error("Height is zero");
    }
    ...
    if compression_method != 0 {
        return format_error("Unrecognized compression method");
    }
    let filter_method = chunk.read_u8()?;
    if filter_method != 0 {
        return format_error("Unrecognized filter method");
    }
